...
of objects that is used by the `query` method to generate
a stream of `CloseApproach` objects that match all of the
desired criteria. The arguments to `create_filters` are
provided by the main module and originate from the user's
command-line options.

This function can be thought to return a collection of
instances of subclasses of `AttributeFilter` - a 1-argument
callable (on a `CloseApproach`) constructed
from a comparator (from the `operator` module), a reference
value, and a class method `get` that subclasses can override
to fetch an attribute of interest from the supplied `CloseApproach`.

The `limit` function simply limits the maximum number of
...
class Hazadousfilter(AttributeFilter):
    """A subclass for filters on hazardous."""
...
class DateFilter(AttributeFilter):
    """A subclass for filters on date."""
...
def create_filters(
    ...
    Each of these arguments is provided by the main module
    with a value from the    user's options at the command line.
    Each one corresponds to a different type of filter. For
    example, the `--date` option corresponds to the `date`
    argument, and represents a filter that selects close approaches
    that occurred on exactly that given date. Similarly, the
    `--min-distance` option corresponds to the `distance_min`
    argument, and represents a filter that selects close approaches
    whose nominal approach distance is at least that far away from
    Earth. Each option is `None` if not specified at the command
    ...
    if diameter_max is not None:
        filter = Diameter_Filter(operator.le, diameter_max)
        filters.append(filter)

    if diameter_min is not None:
        filter = Diameter_Filter(operator.ge, diameter_min)
        filters.append(filter)

    if hazardous is not None:
        filter = Hazadousfilter(operator.eq, hazardous)
        filters.append(filter)
